<template>
	<div>
		...
		<div class="tab-box">
			...
			<!-- 面板左侧导航菜单 -->
			<div class="tab-title">
				<div :class="tabActive==index?'tab-item-title tab-active':'tab-item-title'" v-for="(item,index) in tabList" :key="index"
			...
			<div class="tab-item">
				...
				<div v-if="tabActive == 0">
					<flow-panel-node-res :tabList="tabList"></flow-panel-node-res>
				</div>
				<!--权限配置右侧-->
				<div v-if="tabActive == 1">
					<flow-panel-node-auth :tabList="tabList">
					</flow-panel-node-auth>
				</div>
				<!--行为配置-->
...
<script>
	import FlowPanelNodeRes from '../components/FlowPanelNodeRes.vue'
	import FlowPanelNodeAuth from '../components/FlowPanelNodeAuth.vue'
	import FlowPanelNodeAction from '../components/FlowPanelNodeAction.vue'

	export default {
		name: "FlowNodePanel",
		props: ['graph','graph_id', 'graph_lable', 'graph_type'],
		components: {
			FlowPanelNodeRes,
			FlowPanelNodeAuth,
			FlowPanelNodeAction,
